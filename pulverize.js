...
// Used to keep track of what index of file we're on in the queue
var counter = 0;
...
if (args.length === 0 && options.input.length === 0) {
	respond('help');
} else {
...
function queue (filename) {
	...
	if (task) {
		...
		taskQueue.push(function () {
			return compress(filename, task, promise);
		});
...
function compress (filename, task, promise) {
	var tmp = generateFilename();
	...
	exec('cp "' + filename + '" ' + tmp, function (error, stdout, stderr) {
		task.call(this, tmp, promise).then(function (newStats) {
			...
			if (options.verbose) {
				var index = '[' + ++counter + '/' + report.fileCount + '] ';
				
				if (newStats.size < oldStats.size) {
					sys.puts(index + 'Compressed ' + filename + ' from ' + oldStats.size + ' bytes down to ' + newStats.size + ' bytes');
				} else {
					sys.puts(index + filename + ' could not be compressed any more');
				}
			}

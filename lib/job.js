...
Job.prototype.add = function (filename) {
  ...
  var task = tasks[Job.extensions[path.extname(filename).substr(1)]];
...
Job.prototype.compress = function (filename, task) {
  ...
  var scratchSpace = temp.openSync('PULVERIZR');
  
  // Copy the old file to a temporary spot
  exec('cp "' + filename + '" ' + scratchSpace.path, function (error, stdout, stderr) {
    task.call(this, {
      original: filename,
      path: scratchSpace.path,
      ...
      if (!self.settings.dryRun && newStats.size < oldStats.size) {
        fs.writeFileSync(filename, fs.readFileSync(scratchSpace.path));
      }
      
      fs.closeSync(scratchSpace.fd);
      fs.unlinkSync(scratchSpace.path);

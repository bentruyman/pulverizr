...
Job.prototype = {
	...
	compress: function (filename, task) {
		...
		exec('cp "' + filename + '" ' + scratchSpace, function (error, stdout, stderr) {
			task.call(this, scratchSpace, self.settings, promise).then(function (newStats) {
	...
	run: function () {
		...
		promise.seq(this.queue).then(function () {
			var r = self.report;

			r.time.end = Date.parse(new Date());

			log.info('Finished compressiong job');
			log.info('Scanned ' + r.fileCount + ' files');
		...
		(function (filename, depth) {
			...
			if (stats.isFile()) {
				...
				fs.readdirSync(filename).forEach(function (_filename) {
					...
					callee.call(this, (filename.replace(/(\/)$/g, '') + '/' + _filename), depth);
				});
			}
		}(filename, 0));
